...
* [Grading Workbook Details](#grading-workbook-details)
* [Workbook Builder Details](#workbook-builder-details)
* [Report Generator Details](#report-generator-details)
* [Grading Types](#grading-types)
...
The current version only supports report dissemination through GitHub.  However, if you wanted, you could simply make a hard copy of each report and pass them out in class. (There are tools that will convert markdown to Postscript, html, or pdf.) It should also be straightforward to add code to email each student his or her progress report (either as markdown, or converted into a different format) --- provided there is an email library that will work with your institution's email server.

## Overview of the Major Components

### Grading Workbook

This is the Excel workbook that contains the grades/marks for each assignment. This workbook contains three types of worksheets:
* An info worksheet containing useful student information such as name, username, section number, and GitHub account name
* One worksheet for each grading category (homework, tests, projects, etc.)
* An optional attendance worksheet

See [demo_populated_workbook.xlsx](demo_populated_workbook.xlsx) for an example and the [Grading Workbook](#Grading-workbook-details) section for details.
...
## Grading Workbook Details
Begin by opening this sample, populated grading workbook: 
[demo_populated_workbook.xlsx](demo_populated_workbook.xlsx).
...
In addition to the info worksheet, there is one worksheet for each assignment category. Categories are simply groups of assignments (homework, tests, quizzes, projects, etc.).  For example, look at the worksheet labeled `learningObjectives`.  
* Notice that the leftmost rows of a category worksheet are links to the info sheet.  (If you click on one of these cells, you can see the Excel formula.) Users can hide info columns that are not relevant to a given category worksheet.
* The remaining columns record grades/marks. A mark is simply text. However, each worksheet can be assigned a "grading type", which specifies how the the marks should be interpreted for the purpose of calculating progress and/or the final letter grade. (See [Grading Types](#grading-types) for details.)
...
If you prefer, you can create separate columns for late days and/or comments.  I have found that 
...
**TODO List special marks** (., x, ?etc.)

### Ignoring Columns

If you look at the "Implement" column (column L) in the `learningObjectives` worksheet, you will see that the "short" name in row 2 begins with an "x". The report generator ignores columns whose short names begin with "x". This is useful for (1) adding columns to contain comments (as mentioned above), or (2) suppressing the reporting of assignments that haven't been graded yet.

### Attendance Worksheet

The grading workbook can contain an optional attendance worksheet.

**TODO List attendance marks**

## Workbook Builder Details

Routines in the Poprawa library make assumptions about how the Excel workbook is organized. Therefore, 
although it is possible to simply create a new Excel Workbook "by hand" and populate it, using `workbook_builder`is less error-prone.  In addition, `workbook_builder` will 
...
* `---merge` will let you provide additional config files. (This feature is intended primarily to facilitate testing; but, I can see it being potentially useful for making a quick one-time change to a config without having to edit or copy the "main" config file. It could also be useful for maintaining data that is common to all courses, such as the start and end dates.)
   * Values in subsequent config files override existing values.
   * If a value passed to `merge` begins with `{` it is assumed to be Ruby code. Otherwise, the value is assumed to be a file containing Ruby code. (Again, added primarily to simplify end-to-end testing.)
   * The Ruby code passed should evaluate to a Hash.
...
Open this sample user info `.csv`: [demo/demo_student_roster.csv](demo_student_roster.csv)
...
This syntax is a bit unusual: `info_sheet_config` is an array. Each item in the array is a `Hash` that provides the values in the two header rows. The key is the "short name" and the value is the "long name". (Compare the values in the code above to the header rows in the demo workbook.) Each `Hash` must have exactly one key/value pair.
...
Each category describes one worksheet in the workbook. Typically each worksheet contains a different category of marks (homework, project, learning objective, etc.)  Each category is described by:
  * `key`:         the name of the worksheet (both programmatically and as displayed on the worksheet tabs in Excel)
  * `title`:       the category's full name displayed in reports
...
_Report Generator Details_

## Grading Types

By default, the report generator will just show the text entered in "mark" portion of each cell. However, 
each worksheet can have a "grading type" assigned which determines, 

  1. How the mark is displayed in the progress report
  2. How the mark affects the progress report and/or final letter grade. 

Poprawa currently supports these grading types:

  * **empn**: This is the metric promoted by Robert Talbert on his Grading For Growth blog (**TODO Add link**). Each submission is marked as _E_xceeds Expectations, _M_eets Expectatins, _P_rogressing, or _N_ot Yet. Students may re-attempt assignments without penalty. Thus, the "mark" section of the cell is a sequence of "e"s, "m"s, "p"s, and "n"s. Progress reports show this sequence with the highest mark in bold. 

  * **empn2**: This version of EMPN requires students to achieve a given level of proficiency twice. For example, a student must earn an "e" on two separate submissions to receive an "e" overall. 

  * **letter**: This type simply treats each mark as a letter grade.
