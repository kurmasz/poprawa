...
(async () => {
    ...
    const progress_thresholds = data.thresholds;
    const meets_expectations = data.grades.mastered;
    const progressing = data.grades.progressing;
    const attempted = data.grades.assigned;
    const output_file = data.output_file;

    // determine the student's next grade
    let nextGrade
    for (const key in progress_thresholds.meets_expectations) {
        if (meets_expectations > progress_thresholds.meets_expectations[key])
            break
        nextGrade = key
    }

    // determine how many Ps should be counted towards student's next grade
    let countedP = progress_thresholds.progressing[nextGrade] - progress_thresholds.meets_expectations[nextGrade]
                ...
                x: {
                    ...
                    ticks: {
                        ...
                        callback: function (value) {
                            mArray = Object.entries(progress_thresholds.meets_expectations)
                            newArray = []
